...
const MovementHistoryModal: React.FC<MovementHistoryModalProps> = ({ isOpen, onClose, product, movements, onSaveMovement }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const changeAmount = type === 'Venta' ? -Math.abs(change) : Math.abs(change);
    
    onSaveMovement(product.id, selectedVariantId, {
      timestamp: Date.now(),
      type,
      change: changeAmount,
      notes,
        ...
        <div className="md:col-span-1 space-y-4">
          ...
          <form onSubmit={handleSave} className="p-4 bg-gray-900/50 rounded-lg space-y-4 border border-gray-700">
            ...
            <div>
              ...
              <input type="number" value={change} onChange={(e) => setChange(parseInt(e.target.value, 10))} className="mt-1 w-full bg-gray-700 border-gray-600 rounded py-1 px-2 text-sm" />
            </div>
            <div>
          ...
          <div className="bg-gray-900/50 rounded-lg p-2 border border-gray-700 max-h-96 overflow-y-auto">
            {history.length > 0 ? (
              <ul className="space-y-2">
                {history.map(mov => (
                  <li key={mov.id} className="grid grid-cols-12 items-center gap-2 p-2 bg-gray-800 rounded">
                    ...
                    <div className="col-span-6">
                      ...
                      <p className="text-xs text-gray-400">{mov.notes || new Date(mov.timestamp).toLocaleString('es-CO')}</p>
                    </div>
                    <div className="col-span-5 text-right">
                      <p className="text-sm text-gray-500">Stock Anterior: {mov.newStock - mov.change}</p>
                      <p className="font-bold text-lg">Nuevo Stock: {mov.newStock}</p>
                    </div>
                  </li>
                ))}
              </ul>
