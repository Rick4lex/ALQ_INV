        ...
        {!isFusionMode && !isSelectionMode && (
          <div className={`${viewMode === 'grid' ? 'mt-4 pt-4 border-t' : 'mt-2 pt-2 border-t'} border-gray-700 flex flex-wrap items-center justify-end ${viewMode === 'grid' ? 'gap-x-4 gap-y-2' : 'gap-2'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={(e) => { e.stopPropagation(); onEdit(product); }} className="p-2 rounded-full text-blue-400 hover:bg-blue-500/20" title="Editar"><Pencil size={18} /></button>
              <button onClick={(e) => { e.stopPropagation(); onDelete(product); }} className="p-2 rounded-full text-red-400 hover:bg-red-500/20" title="Eliminar"><Trash2 size={18} /></button>
            </div>
          </div>
        )}
      </div>
