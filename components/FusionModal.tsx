...
interface FusionModalProps {
  ...
  onMerge: (primaryProductId: string, secondaryProductId: string) => void;
}

const FusionModal: React.FC<FusionModalProps> = ({ isOpen, onClose, productsToFuse, onMerge }) => {
  ...
  const handleMerge = () => {
    ...
    if (window.confirm("¿Estás seguro de que quieres fusionar estos dos productos? El producto secundario será eliminado. Esta acción es irreversible.")) {
      onMerge(primaryProductId, secondaryProductId);
    }
  };
